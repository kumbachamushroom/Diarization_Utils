...
def get_segment_list(file_id, segment_dir):
    segments = [(round(float(line.split()[0]),2), round(float(line.split()[1]),2)) for line in open(segment_dir+'/{}.lab'.format(file_id)) if round(float(line.split()[1]),2) - round(float(line.split()[0]),2) > 0.1]
    #split_segs=[]
    #for segment in segments:
    #    if segment[1] - segment[0] > 1:
    #        split_segs.append((segment[0], segment[0]+1))
    #        split_segs.append((float(segment[0])+1,segment[1]))
    #    else:
    #        split_segs.append(segment)
    return segments


def write_track_manifest(audio_path, frame_list, segment_id, manifest_file):
    with open(manifest_file, 'a') as outfile:
        for i in range(len(frame_list)):
            start, stop = frame_list[i][0], frame_list[i][1]
...
@hydra.main(config_path='SpeakerNet_SegmentWise_Diarisation_EMRAI.yaml')
def main(cfg: DictConfig) -> None:
    ...
    for i, id in enumerate(fileids):
        ...
        for j, segment in enumerate(segments):
            start, end = segment[0], segment[1]
            frames = get_frame_list(start, end, 0.6, 0.48)
            frame_list.append(frames)
            # print(frames)
            write_track_manifest(audio_path=cfg.audio.target_path + '{}.wav'.format(id), frame_list=frames,
                                 segment_id=j, manifest_file=cfg.audio.track_manifest)
        ...
        for k, segment in enumerate(segments):
            ...
            try:
                seg_emb = [emb for emb in all_track_embeddings[min(indices):max(indices)+1]]
            except:
                None
            for emb in seg_emb:
        ...
        all_frames = []
        for k in enumerate(frame_list):
            for frame in list(frame_list[k]):
                all_frames.append(frame)
        print(len(all_frames))
        print(len(segment_embs))
